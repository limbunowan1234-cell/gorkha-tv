...
// Gates every /api/follows/* route behind a valid viewer session, same
// pattern as functions/api/favourites/_middleware.js.
export async function onRequest(context) {
  const { request, env, next, data } = context;
  const cookies = parseCookies(request.headers.get('Cookie'));
  const user = await getSessionUser(env.DB, cookies[VIEWER_SESSION_COOKIE]);

  if (!user) return errorResponse('Sign in to follow creators.', 401);
